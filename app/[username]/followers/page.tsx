...
import { getFollowersOf } from '@/lib/ruehl/queries/follows';
...
export default async function FollowersPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const initial = await getFollowersOf(profile.id, 0, 20, supabase);

  return (
    <div className="min-h-screen bg-black text-white">
      <div className="mx-auto max-w-2xl px-4 py-6">
        <FollowListHeader profile={profile} listType="followers" />
        <FollowList targetUserId={profile.id} listType="followers" initial={initial} />
      </div>
    </div>
  );
}
